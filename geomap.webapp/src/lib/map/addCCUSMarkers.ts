...
export const addCCUSMarkers = (
  data: GeoJSONFeatureCollection,
  map: L.Map,
  ccusCluster: L.MarkerClusterGroup,
  statusColorMap: Record<string, string>,
  setSelectedPlantName: (name: string) => void
) => {
  if (!data || !Array.isArray(data.features)) {
    console.warn('Missing or invalid CCUS data');
    return;
  }

  data.features.forEach((feature) => {
    const props = feature.properties as CCUSItem;
    if (feature.geometry.type !== 'Point' || !props.latitude || !props.longitude) {
      console.warn('Skipping CCUS feature with invalid coordinates:', { feature });
      return;
    }
    ...
    const popupHtml = generatePopupHtml(props, 'CCUS');
