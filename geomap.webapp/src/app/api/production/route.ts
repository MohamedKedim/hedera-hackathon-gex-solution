  ...
  try {
    ...
    try {
      ...
      const originalResult = await client.query(
        ...
        LIMIT 1
        `,
        [internal_id, 'Production']
      );

      if (originalResult.rows.length === 0) {
        console.error('No Production record found for internal_id', { internal_id });
        return NextResponse.json({ error: `No Production record found for internal_id: ${internal_id}` }, { status: 404 });
      }
      ...
      const insertResult = await client.query(
        ...
        RETURNING id
        `,
        [internal_id, cleanData, file_link, tab || 'Production', line, sector || 'Production', userEmail, userName, userEmail, userName]
      );

      const newId = insertResult.rows[0].id;
      console.log('New Production record created', { 
        id: newId, 
        internal_id, 
    ...
    console.error('Error processing Production POST request:', error);
