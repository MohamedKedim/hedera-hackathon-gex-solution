  ...
  try {
    ...
    try {
      ...
      if (originalResult.rows.length === 0) {
        console.error('No CCUS record found for internal_id', { internal_id });
        return NextResponse.json({ error: `No CCUS record found for internal_id: ${internal_id}` }, { status: 404 });
      }
      ...
      const insertResult = await client.query(
        ...
        RETURNING id
        `,
        [internal_id, cleanData, file_link, tab || 'CCUS', line, sector || 'CCUS', userEmail, userName, userEmail, userName]
      );

      const newId = insertResult.rows[0].id;
      console.log('New CCUS record created', { id: newId, internal_id });
    ...
    console.error('Error processing CCUS POST request:', error);
