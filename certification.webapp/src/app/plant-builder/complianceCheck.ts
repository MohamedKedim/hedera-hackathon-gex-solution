...
import type { PlacedComponent, Connection } from "@/components/plant-builder/Canvas";
...
export const calculateCompliance = (
  ...
  products.forEach((product) => {
    if (!verifiedProducts.includes(product.productName)) {
      return;
    }
      ...
      .forEach((scheme) => {
        ...
        if (scheme.criteria.carbonFootprint && carbonFootprint > 0) {
          ...
          carbonFootprintDetails = carbonFootprintStatus === "match"
            ? `Value: ${carbonFootprint} ${scheme.criteria.carbonFootprint.unit} (Max: ${scheme.criteria.carbonFootprint.max})`
            : `Value: ${carbonFootprint} ${scheme.criteria.carbonFootprint.unit} exceeds max (${scheme.criteria.carbonFootprint.max})`;
        } else {
        ...
        if (product.offtakeLocation?.country) {
          const marketMatch = scheme.criteria.markets.includes(product.offtakeLocation.country);
          confidenceScore *= marketMatch ? 1 : 0.5;
          eligibleMarkets = marketMatch ? [product.offtakeLocation.country, ...scheme.criteria.markets] : scheme.criteria.markets;
        ...
        results.push({
          ...
          attributes: {
            ...
            carbonFootprint: {
              ...
              value: carbonFootprint > 0 ? carbonFootprint : "N/A",
