...
# ECS Task Definition
resource "aws_ecs_task_definition" "geomap" {
  ...
  container_definitions = jsonencode([
    {
      ...
      image = "${aws_ecr_repository.geomap.repository_url}:latest"
...
# ECS Service
resource "aws_ecs_service" "geomap" {
  name            = "${var.app_name}-service"
  cluster         = aws_ecs_cluster.geomap.id
  task_definition = aws_ecs_task_definition.geomap.arn
...
resource "aws_cloudwatch_log_group" "geomap" {
